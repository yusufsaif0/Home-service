...
{ 
	global $conn;
	if(!isset($_GET['cat'])){
 $sql="select * from electrician where e_title='switch'";
...
{ 
	...
	if(isset($_GET['cat'])){
		$e_title=$_GET['cat'];
 $sql="select * from electrician where e_title='$e_title'";
 $query=mysqli_query($conn,$sql);
 while($run=mysqli_fetch_assoc($query))
 {
$title=$run['e_title'];
...
<div class='col-md-8'>
<h5> $desc</h5>
<a href='electrician.php?cart=$title && price=$price && description=$desc'><button class='btn btn-default'>Add to cart</button></a>
</div>
</div>
</div>
";
