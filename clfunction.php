...
{ 
	global $conn;
	if(!isset($_GET['cat'])){
 $sql="select * from cleaning where c_title='car cleaning'";
...
<div class='col-md-8'>
<h5><b>$desc</b></h5>
 <a href='cleaning.php?cart=$title && price=$price && description=$desc'><button class='btn btn-default'>Add to cart</button></a></div>
</div>
</div>
";
}
...
{ 
	...
	if(isset($_GET['cat'])){
		$c_title=$_GET['cat'];
 $sql="select * from cleaning where c_title='$c_title'";
 $query=mysqli_query($conn,$sql);
 while($run=mysqli_fetch_assoc($query))
 {
$title=$run['c_title'];
