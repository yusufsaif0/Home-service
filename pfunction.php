...
{ 
	global $conn;
	if(!isset($_GET['cat'])){
 $sql="select * from plumber where p_title='Basin & sink'";
...
<div class='col-md-8'>
<h5><b>$desc</b></h5>
 <a href='plumber.php?cart=$title && price=$price && description=$desc'><button class='btn btn-default'>Add to cart</button></a></div>
</div>
</div>
";
}
...
{ 
	...
	if(isset($_GET['cat'])){
		$p_title=$_GET['cat'];
 $sql="select * from plumber where p_title='$p_title'";
 $query=mysqli_query($conn,$sql);
 while($run=mysqli_fetch_assoc($query))
 {
$title=$run['p_title'];
...
<a href='plumber.php?cart=$title && price=$price && description=$desc'><button class='btn btn-default'>Add to cart</button></a>
</div>
</div>
</div>
";
