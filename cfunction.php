...
{ 
	global $conn;
	if(!isset($_GET['cat'])){
 $sql="select * from carpenter where c_title='balcony'";
...
<div class='col-md-8'>
<h5> $desc</h5>
 <a href='carpenter.php?cart=$title && price=$price && description=$desc'><button class='btn btn-default'>Add to cart</button></a></div>
</div>
</div>
";
}
...
{ 
	...
	if(isset($_GET['cat'])){
		$c_title=$_GET['cat'];
 $sql="select * from carpenter where c_title='$c_title'";
 $query=mysqli_query($conn,$sql);
 while($run=mysqli_fetch_assoc($query))
 {
$title=$run['c_title'];
