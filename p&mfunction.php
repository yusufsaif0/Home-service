...
{ 
	global $conn;
	if(!isset($_GET['cat'])){
 $sql="select * from packers_and_movers where p_title='switch'";
...
<h5>Description $desc</h5>
 <a href='electrician.php?cart=$title,price=$price && description=$desc'><button class='btn btn-default'>Add to cart</button></a></div>
</div>
</div>
";
}
...
{ 
	...
	if(isset($_GET['cat'])){
		$p_title=$_GET['cat'];
 $sql="select * from packers_and_movers where p_title='$p_title'";
 $query=mysqli_query($conn,$sql);
 while($run=mysqli_fetch_assoc($query))
 {
$title=$run['p_title'];
...
<div class='col-md-8'>
<h5>Description $desc</h5>
<a href='carpenter.php?cart=$title && price=$price && description=$desc'><button class='btn btn-default'>Add to cart</button></a>
</div>
</div>
</div>
";
